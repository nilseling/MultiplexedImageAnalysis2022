...
For this, we will use the [cytomapper](https://www.bioconductor.org/packages/release/bioc/html/cytomapper.html) package. 
We will consistently visualize the four images randomly sampled above.
...
In the first example, we will vizualice E-cadherin (tumor cells), Ki67 (proliferation marker),
CD3 (T cells) and CD20 (B cells) and a nuclear marker.

```{r 5-color-composites, message=FALSE}
...
```{r outline-t-cells, message=FALSE}
plotPixels(images[cur_img_ids],
